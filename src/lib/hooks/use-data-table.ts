...
export function useDataTable<TData, TValue>({
  ...
  const { searchableColumns, filterableColumns, dateRangeColumns } =
    React.useMemo(() => {
      return {
        ...
        dateRangeColumns: filterFields.filter(
  ...
  }, [filterableColumns, searchableColumns, dateRangeColumns, searchParams])
  ...
  const dateRangeColumnFilters = columnFilters.filter((filter) => {
    return dateRangeColumns.find((column) => column.value === filter.id)
  })
  ...
  React.useEffect(() => {
    ...
    // Handle date range column filters
    for (const column of dateRangeColumnFilters) {
      if (typeof column.value === "string") {
        Object.assign(newParamsObject, {
          [column.id]: typeof column.value === "string" ? column.value : null,
        })
    ...
    for (const key of searchParams.keys()) {
      if (
        ...
        (dateRangeColumns.find((column) => column.value === key) &&
          !dateRangeColumnFilters.find((column) => column.id === key))
      ) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    JSON.stringify(dateRangeColumnFilters),
  ])
